...
public static class GESaveDialogA
{
	public static UIC Assemble(UIC _parent)
	{
		return Assemble(_parent, false);
	}

	public static UIC Assemble(UIC _parent, bool network)
	{
		string[] tags = new string[1] { "Save" };
		UIC uIC = CanvasA.Assemble(Main.uiCamera, -1, "Save As...", null, tags);
		...
		if (network)
		{
			...
			string[] folderContents = FTP.GetFolderContents("public_html/alteration/_L/" + Main.m_currentGame.m_projectCode + "/");
		}
		else
		{
			...
			for (int i = 0; i < customLevelFolderContents.Length; i++)
			{
				...
				{
					string label = array[array.Length - 2];
					UIC uIC3 = ButtonA.Assemble(uIC2.canvasCamera, i, label, HandleFileList, tags);
		...
		UIC uIC4 = TextFieldA.Assemble(Main.uiCamera, 100003, "Chapter Index", null, TextFieldA.HandleTouches, tags);
		UIC uIC5 = TextFieldA.Assemble(Main.uiCamera, 100004, "Level Index", null, TextFieldA.HandleTouches, tags);
		UIC uIC6 = TextFieldA.Assemble(Main.uiCamera, 100002, "File", null, TextFieldA.HandleTouches, tags);
		UIS.AddToCanvasGrid(uIC4, uIC, true);
		UIS.AddToCanvasGrid(uIC5, uIC, false);
		UIS.AddToCanvasGrid(uIC6, uIC, false);
		UIS.SetRelativeSize(uIC4, 0.25f, 0.1f);
		UIS.SetRelativeSize(uIC5, 0.25f, 0.1f);
		UIS.SetRelativeSize(uIC6, 0.4f, 0.1f);
		TextFieldA.DrawTextField(uIC4, LevelManager.m_currentLevel.chapterIndex.ToString());
		TextFieldA.DrawTextField(uIC5, LevelManager.m_currentLevel.levelIndex.ToString());
		TextFieldA.DrawTextField(uIC6, LevelManager.m_currentLevel.name);
		UIC uIC7 = ButtonA.Assemble(Main.uiCamera, 0, "Save", HandleOpenWindow, tags);
		UIC uIC8 = ButtonA.Assemble(Main.uiCamera, 0, "Cancel", HandleOpenWindow, tags);
		UIS.AddToCanvasGrid(uIC7, uIC, true);
		UIS.AddToCanvasGrid(uIC8, uIC, false);
		UIS.SetRelativeSize(uIC7, 0.3f, 0.08f);
		UIS.SetRelativeSize(uIC8, 0.3f, 0.08f);
		DrawButton(uIC7);
		DrawButton(uIC8);
	...
	private static void HandleFileList(TouchAreaC _c, int _i, bool _consumed)
	{
		...
		UIC uIComponentByIdentifier = UIS.GetUIComponentByIdentifier(100002);
	...
	public static void HandleOpenWindow(TouchAreaC _c, int _i, bool _consumed)
	{
		UIC uIC = _c.customComponent as UIC;
		...
		if (_c.identifier == "Save")
		{
			string name = LevelManager.m_currentLevel.name;
			uint chapterIndex = LevelManager.m_currentLevel.chapterIndex;
			uint levelIndex = LevelManager.m_currentLevel.levelIndex;
			uint result = 0u;
			uint.TryParse(UIS.GetUIComponentByIdentifier(100003).textC.text, out result);
			uint result2 = 0u;
			uint.TryParse(UIS.GetUIComponentByIdentifier(100004).textC.text, out result2);
			string text = UIS.GetUIComponentByIdentifier(100002).textC.text;
			if (result2 != 0)
			{
				text = "c" + result + "l" + result2;
			}
			LevelManager.m_currentLevel.name = text;
			LevelManager.m_currentLevel.chapterIndex = result;
			LevelManager.m_currentLevel.levelIndex = result2;
			LevelManager.SaveLevelData(LevelManager.m_currentLevel as GELevel, text);
			LevelManager.m_currentLevel.name = name;
			LevelManager.m_currentLevel.chapterIndex = chapterIndex;
			LevelManager.m_currentLevel.levelIndex = levelIndex;
			string path = IO.GetApplicationRunPath() + "/Levels/" + text + ".bytes";
			LevelManager.ChangeLevel(new GELevelResource(null, text, path, ResourceType.Level), true);
