...
public static class GEFlickableControllerA
{
	public static GETriggerC Assemble(EIC _eic, TriggerData _data)
	{
		...
		if (_eic.container != null && _eic.container.identifier == "Block")
		{
			...
			PrefabS.CreatePathPrefabComponentFromPolygon(transformC, Vector3.forward * -2f, polygon, 6f, DebugDraw.GetColor(50f, 128f, 255f, 255f), ResourceManager.GetMaterial("Line6"), Main.camera, Position.Center, true);
		}
		return gETriggerC;
	}
	...
	public static void PopulatePropertyBar(EIC _eiC, UIC _propertyBar)
	{
		...
		UIC component = NumericFieldA.Assemble(Main.uiCamera, "Flick Strength", HandleEventPropertyChange, null, true, Align.Left, 80f, 1f, false, 0f, 9999f, triggerData.eventDispatchDelay, tags);
		UIS.AddToCanvasGrid(LabelA.Assemble(canvasCamera, "Flickable", tags), _propertyBar, true);
	...
	private static void HandleTouches(TouchAreaC _c, int _i, bool _consumed)
	{
		...
		if (_c.touchEvent[_i] == TouchEvent.RollIn)
		{
			...
			zero = InputManager.m_touches[_c.touchIndex[_i]].deltaPosition;
