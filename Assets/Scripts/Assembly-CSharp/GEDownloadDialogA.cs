...
public static class GEDownloadDialogA
{
	...
	public static UIC Assemble(UIC _parent)
	{
		...
		UIC uIC = CanvasA.Assemble(Main.uiCamera, -1, "Download from server...", null, tags);
		...
		for (int k = 0; k < folderContents.Length; k++)
		{
			string[] array2 = folderContents[k].Split(separator, StringSplitOptions.None);
		...
		UIC uIC6 = ButtonA.Assemble(Main.uiCamera, 0, "Download", HandleDownloadWindow, tags);
		UIC uIC7 = ButtonA.Assemble(Main.uiCamera, 0, "Cancel", HandleDownloadWindow, tags);
	...
	public static void HandleDownloadWindow(TouchAreaC _c, int _i, bool _consumed)
	{
		UIC uIC = _c.customComponent as UIC;
		...
		if (_c.identifier == "Download")
		{
			...
			{
				string[] array = folderContents[i].Split(separator, StringSplitOptions.None);
			...
			if (flag)
			{
				...
				string resourceLevelPath = IO.GetResourceLevelPath();
				resourceLevelPath = resourceLevelPath + "/" + text2 + ".bytes";
				FTP.Download("public_html/alteration/_L/" + Main.m_currentGame.m_projectCode + "/" + text2 + ".bytes", resourceLevelPath);
