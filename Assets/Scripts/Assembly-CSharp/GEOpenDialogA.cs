...
public static class GEOpenDialogA
{
	private static string[] builtInLevelNames;

	public static UIC Assemble(UIC _parent)
	{
		string[] tags = new string[1] { "Open" };
		UIC uIC = CanvasA.Assemble(Main.uiCamera, -1, "Open...", null, tags);
		...
		UIC uIC6 = ButtonA.Assemble(Main.uiCamera, 0, "Open", HandleOpenWindow, tags);
	...
	public static void HandleOpenWindow(TouchAreaC _c, int _i, bool _consumed)
	{
		...
		if (_c.identifier == "Open")
		{
			...
			if (flag)
			{
				...
				LevelManager.ChangeLevel(new GELevelResource(null, text2, path, ResourceType.Level), true);
