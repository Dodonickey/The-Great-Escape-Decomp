...
public static class GEAppendDialogA
{
	private static string[] builtInLevelNames;

	public static UIC Assemble(UIC _parent)
	{
		string[] tags = new string[1] { "Append" };
		UIC uIC = CanvasA.Assemble(Main.uiCamera, -1, "Append...", null, tags);
		...
		TextFieldA.DrawTextField(uIC5, string.Empty);
		UIC uIC6 = ButtonA.Assemble(Main.uiCamera, 0, "Append", HandleOpenWindow, tags);
		UIC uIC7 = ButtonA.Assemble(Main.uiCamera, 0, "Cancel", HandleOpenWindow, tags);
	...
	public static void HandleOpenWindow(TouchAreaC _c, int _i, bool _consumed)
	{
		...
		if (_c.identifier == "Append")
		{
			...
			if (flag)
			{
				...
				string path = IO.GetApplicationRunPath() + "/Levels/" + text2 + ".bytes";
				LevelManager.AppendLevel(new GELevelResource(null, text2, path, ResourceType.Level), true);
				EditorState.ResetOutliner();
				return;
			}
			string[] resourceLevelFolderContents = IO.GetResourceLevelFolderContents();
			flag = false;
			string text3 = UIS.GetUIComponentByIdentifier(100001).textC.text;
			for (int j = 0; j < builtInLevelNames.Length; j++)
			{
				text3 = builtInLevelNames[j];
				if (text3 + " (build)" == UIS.GetUIComponentByIdentifier(100001).textC.text)
				{
					flag = true;
					break;
				}
			...
			{
				GEMenuAreaA.CloseMenu();
				LevelManager.ChangeLevel(new GEBuiltinLevelResource(text3, Main.m_currentGame.m_projectCode + "/Levels/" + text3, ResourceType.Level), true);
				EditorState.ResetOutliner();
			}
		}
